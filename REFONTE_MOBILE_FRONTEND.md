...
| **Selects (formulaire)** | Risque de double-flèche / style natif si `-webkit-appearance` absent | Vérifié : **autoprefixer** (defaults) émet bien `-webkit-appearance:none` dans le CSS compilé → OK, + chevron custom déjà présent |
| **Select mois/année du calendrier** | `.guest-calendar-select` avait `padding`/`border` custom **sans** `-webkit-appearance:none` → iOS Safari clippe le texte du rendu natif → **le mois n'était pas visible** | Ajout `-webkit-appearance:none` + chevron custom + `height:40px`/`line-height` + `overflow:hidden;text-overflow:ellipsis` ([mobile.css](src/styles/mobile.css)) |

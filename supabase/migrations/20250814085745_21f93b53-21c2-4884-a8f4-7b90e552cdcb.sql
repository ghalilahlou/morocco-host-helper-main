-- Fixed comprehensive security reversal migration
...
DROP POLICY IF EXISTS "Users can manage reservations for their properties" ON public.airbnb_reservations;
DROP POLICY IF EXISTS "Users can view sync status for their properties" ON public.airbnb_sync_status;
DROP POLICY IF EXISTS "Users can manage sync status for their properties" ON public.airbnb_sync_status;

-- 3. Drop triggers first (to resolve dependencies)
...
-- 4. Now drop security definer functions
...
-- 5. Remove encrypted columns from guests table and restore plain text columns
...
-- 6. Remove additional security columns if they exist
...
-- 9. Ensure storage policies allow public access
DROP POLICY IF EXISTS "Users can view their own guest documents" ON storage.objects;
DROP POLICY IF EXISTS "Users can upload guest documents" ON storage.objects;
DROP POLICY IF EXISTS "Users can update their guest documents" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete their guest documents" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can update their contracts" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete their contracts" ON storage.objects;
...
CREATE POLICY "Public access to guest documents" ON storage.objects FOR ALL USING (bucket_id = 'guest-documents');
CREATE POLICY "Public access to contracts" ON storage.objects FOR ALL USING (bucket_id = 'contracts');
CREATE POLICY "Public access to signed contracts" ON storage.objects FOR ALL USING (bucket_id = 'signed-contracts');
CREATE POLICY "Public access to property photos" ON storage.objects FOR ALL USING (bucket_id = 'property-photos');
CREATE POLICY "Public access to police forms" ON storage.objects FOR ALL USING (bucket_id = 'police-forms');
-- Fix the v_guest_submissions view to handle tokens without booking_id
...
SELECT 
    ...
    pvt.property_id,
    CASE 
        -- Use token's booking_id if available and cast to UUID
        WHEN pvt.booking_id IS NOT NULL AND pvt.booking_id != '' THEN pvt.booking_id::uuid
        -- Otherwise, try to match with bookings by property_id and check-in date
        ELSE (
            SELECT b.id
            FROM bookings b 

...
// ✅ SÉCURISÉ : Configuration CORS avec checky.ma
...
function getCorsHeaders(request: Request): Record<string, string> {
  ...
  const isAllowedOrigin = origin && (
    ...
    origin.includes('vercel.app') // Fallback pour preview deployments
  );
...
serve(async (req) => {
  // ✅ SÉCURISÉ : Utiliser handleOptions pour CORS preflight
  ...
  // ✅ SÉCURISÉ : Obtenir les headers CORS dynamiques basés sur l'origine
  ...
  try {
    ...
    // ✅ Vérifier les permissions de génération de tokens (avec fallback permissif)
    ...
    // ✅ Vérifier d'abord si un token actif récent existe déjà (idempotence)
    ...
    try {
      ...
      if (existingActiveToken) {
        ...
        if (tokenAge < 5000) {
          ...
          // ✅ FORCÉ : Utiliser checky.ma en production
    ...
    // ✅ Désactiver tous les tokens actifs existants pour cette propriété
    ...
    // ✅ Gestion des codes Airbnb sécurisés avec support des liens directs
    ...
    // ✅ Incrémenter le compteur de réservations (seulement si la fonction existe)
    try {
    ...
    // ✅ FORCÉ : Construire l'URL du lien invité avec checky.ma
    const baseUrl = Deno.env.get('PUBLIC_APP_URL') || Deno.env.get('SITE_URL') || 'https://checky.ma';
...
// ✅ Fonction améliorée pour générer un token unique crypto-secure
